...
@njit
def replaceColor(matrix, palette, myDict):
    ...
    for row in range(width):
        for col in range(height):
            ...
            for each in palette:
                dist = _redmean(r1, g1, b1, myDict[each[1]], myDict[each[2]], myDict[each[3]])
                if dist < closestDist:
                    closestDist = dist
                    #closestDmc = each
...
class Dmc(Boxed):
    ...
    def apply(self, matrix, numSuperPixels):
        ...
        myDict = typed.Dict.empty(key_type=types.unicode_type, value_type=types.int64)
        for x in range(256):
            myDict[str(x)] = x
        ...
        palette = typed.List()
        for each in temp:
            palette.append(typed.List(each))
        return replaceColor(matrix, palette, myDict)
    ...
    #         g2 = int(each[2])
    #         b2 = int(each[3])
    #         dist = _redmean(avg[0], avg[1], avg[2], r2, g2, b2)
    #         if dist < closestDist:
    #             closestDist = dist
    #             r = r2
    #             g = g2
